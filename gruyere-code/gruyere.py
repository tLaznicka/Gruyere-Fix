...
class GruyereRequestHandler(BaseHTTPRequestHandler):
  ...
  def _GenerateXSRFToken(self):
      return os.urandom(24).hex()  # Generates a unique token
    ...
    if 'uid' in params and 'pw' in params:
        ...
        if uid in database:
            if database[uid]['pw'] == self._GetParameter(params, 'pw'):
                ...
                xsrf_token = self._GenerateXSRFToken()  
                specials['xsrf_token'] = xsrf_token  
...
def _DoDeletesnippet(self, cookie, specials, params):
    ...
    Args:
      ...
      params: Cgi parameters.
    """
    # Ověření XSRF tokenu před pokračováním
    submitted_token = self._GetParameter(params, 'xsrf_token')
    if submitted_token != specials.get('xsrf_token'):
        self._SendError('XSRF token invalid or missing.', cookie, specials, params)
        return
